...
def traces_to_examples(valid_traces,invalid_traces,auto):
    res = ''
    penalty = ''
    if(auto.error_num!=0):
        penalty = '@2'
...
def learnAutomata_WithTeacher(auto,learn_id,test_range,test_num,learn_once=False):
    ...
    while(not learned):
        ...
        exps = traces_to_examples(valid_trace,invalid_trace,auto)    

        completeIlaspEncoding(learningTask,exps,len(auto.states))
        
        learning_res = execute_ILASP(learningTask)
        learningSuccess, time =  checkLearningIsRight_ByTest(auto, learning_res,learn_id)
        if(learn_once):
...
def produceMissingErrorGraph():
    ...
    example_number = [5,15,25,40]
    missing_probs = [0.05,0.1,0.2]
    ...
    plt.xlabel('example_number')
    plt.ylabel('accuracy')
    plt.legend(handles=patches)
    plt.show() 
    plot_file = 'err_different_percentage_prob_0.3_state2to4_run50.png'
...
def produceLearnTimeAgainstEdge(run_total):
    ...
    record = {}
    for i in range(run_total):
        st_num = random.choice(num_states)

        auto = Automata(st_num,0,1)
        auto.generate_automata()
        num_edges = len(list( auto.graph.edges()))

        correct, time,_ = learnAutomata_WithTeacher(auto,st_num*10000+i,1000,10*st_num,learn_once=False)

        if(not num_edges in record):
            record[num_edges] = [time]
        else:
            record[num_edges].append(time)
    ...
    for k in record.keys():
        ...
        num_edges.append(k)
        min_learn.append(shortest)
        max_learn.append(longest)
        avg_learn.append(avg)
        print 'number of automatas with edge number {} : {}'.format(k,len(rec))


    plt.clf()
    plt.xlabel('Number of Deltas')
    plt.ylabel('Average Learning time')
    plt.plot(num_edges,avg_learn,'b'+draw_style)
    plot_file = 'learning_time_against_deltas_avg.png'
    plt.savefig(plot_file)


    plt.clf()
    plt.xlabel('Number of Deltas')
    plt.ylabel('Longest Learning time')
    plt.plot(num_edges,max_learn,'r'+draw_style)
    plot_file = 'learning_time_against_deltas_long.png'
    plt.savefig(plot_file)


    plt.clf()
    plt.xlabel('Number of Deltas')
    plt.ylabel('Shortest Learning time')
    plt.plot(num_edges,min_learn,'y'+draw_style)
    plot_file = 'learning_time_against_deltas_short.png'
    plt.savefig(plot_file)

    plt.clf()
    plt.xlabel('Number of Deltas')
    plt.ylabel('Learning time')
    plt.plot(num_states,avg_learn,'b'+draw_style)
    plt.plot(num_states,max_learn,'r'+draw_style)
    plt.plot(num_states,min_learn,'y'+draw_style)
    patches = []
    patches.append(mpatches.Patch(color='b',label='avg learn time' ))
    patches.append(mpatches.Patch(color='r',label='max learn time' ))
    patches.append(mpatches.Patch(color='y',label='min learn time' ))
    plt.legend(handles = patches)
    plot_file = 'learning_time_against_deltas_all.png'
    plt.savefig(plot_file)
...
def produceLearnTimeAgainstState(run_total):
    ...
    plt.ylabel('Longest Learning time')
    plt.plot(num_states,max_learn,'r'+draw_style)
    plot_file = 'learning_time_against_states_long.png'
    plt.savefig(plot_file)
    ...
    plt.ylabel('Shortest Learning time')
    plt.plot(num_states,min_learn,'y'+draw_style)
    plot_file = 'learning_time_against_states_short.png'
    plt.savefig(plot_file)
    ...
    patches.append(mpatches.Patch(color='y',label='min learn time' ))
    plt.legend(handles = patches)
    plot_file = 'learning_time_against_states_all.png'
    plt.savefig(plot_file)
...
produceLearnTimeAgainstState(50)
