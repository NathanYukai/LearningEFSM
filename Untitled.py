...
class RegAutomata:
    ...
    def generate_automata(self,dst):
        ...
        end_reg_value = random.randint(0,reg_limit)
        final_state = self.state_num-1
        
        graph = getValidGraph(self)
        deltas = list(graph.edges())
        
        deltas_str = genRegDeltaConditions(deltas,graph)
        ...
        extra += ':- not st(T,state{}),trace_length(T).\n'.format(final_state)       
        extra += ':- not reg(T,state{},{}), trace_length(T).\n'.format(final_state,end_reg_value)
        print extra
...
#                 auto.setMissingInput(missing_probs[mp],1)
                ...
                correct, time = learnAutomata_WithTeacher(auto,st_num*100+i,1000,en*st_num,learn_once=True)
                if(correct):
                    correct_num += 1
                
                idx = num_states.index(st_num)
                if(i==0):
                    learn_time_min.append(time)
                    learn_time_max.append(time)
                learn_time_min[idx] = min(time,learn_time_min[idx])
                learn_time_max[idx] = max(time,learn_time_max[idx])
                total_time += time
            learning_time.append(total_time/run_total)
...
plt.show() 
plot_file = 'err_different_percentage_prob_0.3_state2to5_run50.png'
plt.savefig(plot_file)



# for i in range(len(num_states)):
#     learn_time_max[i] -= learning_time[i] 
#     learn_time_min[i] = learning_time[i] - learn_time_min[i]
    
# print learning_time, learn_time_max, learn_time_min
# plt.errorbar(num_states,learning_time,yerr = (learn_time_min,learn_time_max))
# plt.xlabel('state number')
# plt.ylabel('avg learning time, with min, max time')
# plt.savefig('state_learning_compare_withBar_to_state_5.png')
# plt.show()
